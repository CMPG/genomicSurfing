...
# lvl=.1
# yli=1.5e-4
plot_graph_per_gen_complete = function(cur_gen, cur_rep, dt, color_troughs=NULL, lvl=NULL, yli=NULL, win_basepair) {
  ...
  if (!is.null(color_troughs)) {
    ...
    tr_dat = calc_troughs(dt, cur_gen, lvl, cur_rep, win_basepair)
...
# changes:
# Wed May 05 13:57:10 2021 ------------------------------

# level_trough=0.1; model_core="f20_m10_g5"; type1="high_rec"; type2="low_rec"; win_size="large"; measure_variable="mean_size"; group_variable="gen"; color1=azul; color2=verde; color3=rosa; log10_transform=F; f_trans=1; resc_ymax=1;

plot_different_models_rec_rate=function(level_trough, model_core, type1, type2, win_size, measure_variable, group_variable, color1, color2, color3, log10_transform=F, f_trans=1, resc_ymax=1){
  
  d1= fread(sprintf("%s/summary_data_lvl_%s_%s_10inds_win_%s.txt", sprintf("./r_%s", model_core), level_trough*100, model_core, win_size))
  d2= fread(sprintf("%s/summary_data_lvl_%s_%s_10inds_win_%s.txt", sprintf("./r_%s_%s", model_core, type1), level_trough*100, model_core, win_size))
  d3= fread(sprintf("%s/summary_data_lvl_%s_%s_10inds_win_%s.txt", sprintf("./r_%s_%s", model_core, type2), level_trough*100, model_core, win_size))
  ...
  if (log10_transform == T){
    ...
    mv_title=unlist(strsplit(mv_label, split = "\\("))[1]
    gv_title=unlist(strsplit(gv_label, split = " \\("))[1]
    plot(x=s1[,group_variable], y=s1[,measure_variable]/f_trans, type="l", col=color1, ylim = c(min_yli/f_trans, yli/f_trans), main=sprintf("%s by %s;", mv_title, gv_title), sub = sprintf("Models: %s, %s, %s;", model_core, type1, type2), xlab = gv_label, ylab = mv_label, cex.main=2, cex.lab=1.5, cex.axis=1.5)

    polygon(c(s1[,group_variable], rev(s1[,group_variable])), c(s1[,'qt5']/f_trans, rev(s1[,'qt1']/f_trans)), col=scales::alpha(color1, 0.01), border = scales::alpha(color1, 0.5))
    
    lines(x=s2[,group_variable], y=s2[,measure_variable]/f_trans, col=color2)
    polygon(c(s2[,group_variable], rev(s2[,group_variable])), c(s2[,'qt5']/f_trans, rev(s2[,'qt1']/f_trans)), col=scales::alpha(color2, 0.01), border = scales::alpha(color2, 0.5))
    
    lines(x=s3[,group_variable], y=s3[,measure_variable]/f_trans, col=color3)
    polygon(c(s3[,group_variable], rev(s3[,group_variable])), c(s3[,'qt5']/f_trans, rev(s3[,'qt1']/f_trans)), col=scales::alpha(color3, 0.001), border = scales::alpha(color3, 0.5))
    plot.new()
    par(oma=c(0,0,0,0))
    legend("right", c(model_core, type1, type2), fill=c(azul, verde, rosa), cex = 2, xpd = NA)
